    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      ...
      >
        <div className="flex items-center gap-4 p-6 border-b border-gray-200 dark:border-gray-700">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">Delete Doctor</h2>
        ...
        <div className="p-6">
          <p className="text-gray-700 dark:text-gray-300 mb-2">
            Are you sure you want to delete <strong>Dr. {doctor?.full_name}</strong>?
          </p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            This will permanently remove the doctor and all their subject assignments from the system.
          </p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-red-600 dark:text-red-400 mt-1">
                  All associated data including subject assignments will be permanently deleted.
                </p>
              </div>
            </div>
          </div>
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-600 dark:text-blue-400">
                {doctor ? "Update doctor information" : "Create a new medical professional account"}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto">
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder={doctor ? "Enter new password" : "Create secure password"}
              />
              <button
                type="button"
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 max-h-48 overflow-y-auto p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-200 dark:border-gray-600">
              {subjects.map((subject) => (
                <label key={subject.id} className="flex items-center space-x-3 p-3 hover:bg-white dark:hover:bg-gray-600 rounded-lg cursor-pointer transition-colors border border-transparent hover:border-blue-200 dark:hover:border-blue-800">
                  <input
                    type="checkbox"
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-center justify-between text-sm mb-4">
          <div className="flex items-center gap-3">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border transition-colors ${
              ...
              {doctor.is_active ? <UserCheck className="w-3 h-3" /> : <UserX className="w-3 h-3" />}
              <span className="text-xs font-semibold">{doctor.is_active ? 'Active' : 'Inactive'}</span>
            </div>
            ...
            <div className="flex items-center gap-1 text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-3 py-1.5 rounded-full border border-gray-200 dark:border-gray-600">
              ...
              <span className="text-xs font-semibold">{doctor.subjects?.length || 0} subjects</span>
            </div>
          </div>
        ...
        {doctor.subjects && doctor.subjects.length > 0 && (
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-3 uppercase tracking-wide">Specializations</p>
            <div className="flex flex-wrap gap-2">
              {doctor.subjects.map((subject) => (
                <span
                  key={subject.id}
                  className="px-3 py-1.5 bg-gradient-to-r from-blue-100 to-blue-200 dark:from-blue-900/30 dark:to-blue-800/30 text-blue-700 dark:text-blue-400 rounded-lg text-xs font-semibold border border-blue-200 dark:border-blue-800 shadow-sm"
                >
                  {subject.name}
                </span>
              ))}
...
export default function ManageDoctors() {
        ...
        if (data.plainPassword) {
          toast.success(
            <div>
              ...
              <p className="text-sm mt-1">Temporary password: <strong>{data.plainPassword}</strong></p>
              <p className="text-xs mt-1 text-gray-600">Share this with the doctor and ask them to change it immediately.</p>
            </div>,
            { duration: 8000 }
          );
        } else {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/doctors/delete?id=${selectedDoctor.id}`, {
        method: 'DELETE'
      });
    ...
    return (
      <div className="flex items-center justify-center min-h-96">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 font-medium text-lg">Loading Medical Team...</p>
          <p className="text-gray-500 dark:text-gray-500 text-sm mt-2">Fetching doctor records</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-6 text-white shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-2">{pagination.totalDoctors}</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-6 text-white shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-100 text-sm font-medium">Avg. Specializations</p>
              <p className="text-3xl font-bold mt-2">
                {doctors.length > 0 
                  ? Math.round(doctors.reduce((acc, doc) => acc + (doc.subjects?.length || 0), 0) / doctors.length) 
                  : 0
                }
              </p>
            </div>
